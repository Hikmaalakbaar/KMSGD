...
export default function Kontak() {
  return (
    <div className="pt-[4.69rem]">
      <section className="relative py-14 overflow-hidden sm:py-20 bg-gradient-to-br from-yellow-600 to-yellow-800">
        <div
        ...
        <div className="relative px-4 mx-auto text-center max-w-7xl sm:px-6 lg:px-8">
          ...
          <p className="max-w-xl mx-auto text-yellow-200">Punya pertanyaan atau ingin bergabung? Kami siap membantu kamu!</p>
        </div>
      </section>
              ...
              <div className="space-y-5">
                {[
                  ...
                  <div key={i} className="flex items-start gap-4 p-5 bg-white shadow-sm rounded-2xl">
                    <div className="flex items-center justify-center w-11 h-11 rounded-xl bg-yellow-50 shrink-0">
                      <c.icon size={18} className="text-black" />
                    </div>
                    <div>
                      <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-0.5">{c.label}</p>
                      {c.href ? (
                        <a href={c.href} className="font-medium transition-colors text-slate-700 hover:text-yellow-500">{c.value}</a>
                      ) : (
              ...
              <form className="space-y-5" onSubmit={e => { e.preventDefault(); alert('Pesan terkirim! Kami akan segera menghubungi kamu.') }}>
                <div>
                  ...
                  <input type="text" placeholder="Masukkan nama lengkap kamu" required
                    className="w-full px-4 py-3 bg-[#F1F5F9] border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-yellow-300 focus:border-yellow-400 transition-colors" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <select className="w-full px-4 py-3 bg-[#F1F5F9] border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-yellow-300 focus:border-yellow-400 transition-colors text-slate-600">
                ...
                <div>
                  ...
                  <textarea rows={4} placeholder="Tulis pesanmu di sini..." required
                    className="w-full px-4 py-3 bg-[#F1F5F9] border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-yellow-300 focus:border-yellow-400 transition-colors resize-none" />
                </div>
                <button type="submit"
                  className="w-full flex items-center justify-center gap-2 bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-3.5 px-6 rounded-xl transition-all duration-200 shadow-lg shadow-yellow-500/25 hover:-translate-y-0.5">
